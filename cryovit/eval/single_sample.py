...
def merge_samples(results_dir: Path) -> pd.DataFrame:
    ...
    for sample in Sample:
        df = pd.read_csv(results_dir / f"{sample.name}.csv")
...
def wilcoxon_test(group, key: str, model_A: str, model_B: str) -> float:
...
def compute_stats(df: pd.DataFrame, key: str, file_name: str) -> pd.Series:
    ...
    """
    grouped = df.groupby(["Sample", key], sort=False)["TEST_DiceMetric"].agg(
        mean="mean",
        std="std",
        median="median",
...
if __name__ == "__main__":
    ...
    exp_names = {
        ...
        "single_sample_unet3d_mito": "3D U-Net",
    }

    test_func = functools.partial(
        wilcoxon_test, key="Model", model_A="CryoViT", model_B="3D U-Net"
    )

    df = merge_experiments(exp_dir, exp_names, key="Model")
    df.to_csv(result_dir / "single_sample_model_raw.csv", index=False)
    pvalues = compute_stats(df, "Model", result_dir / "single_sample_model.csv")

    title = "CryoViT vs 3D U-Net Comparison on Individual Samples"
    plot_df(df, pvalues, "Model", title, result_dir / "single_sample_model")

    ################################################

    exp_names = {
        "single_sample_cryovit_mito": "CryoViT with Sparse Labels",
        "single_sample_cryovit_mito_ai": "CryoViT with Dense Labels",
    }
    ...
    df.to_csv(result_dir / "single_sample_label_raw.csv", index=False)
    pvalues = compute_stats(df, "Label Type", result_dir / "single_sample_label.csv")

    title = "CryoVIT: Sparse vs Dense Labels Comparison on Individual Samples"
    plot_df(df, pvalues, "Label Type", title, result_dir / "single_sample_label")
